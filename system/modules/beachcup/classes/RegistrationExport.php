...
class RegistrationExport
{
    public function export(\Contao\DC_Table $dc)
    {
        ...
        $row = $database->query("SELECT s.id FROM tl_beachcup_stage AS s WHERE (s.start_date <= UNIX_TIMESTAMP() and s.end_date + (2 * 86400) >= UNIX_TIMESTAMP()) or s.start_date >= UNIX_TIMESTAMP() ORDER BY s.start_date ASC LIMIT 1;")->fetchAssoc();
        $stage_id = $row["id"];
        ...
        if(!empty($stage_id))
        {
            $data = $database->query("select tl_beachcup_registration.id as ANMELDUNG_ID, tl_beachcup_tournament.name_de AS TURNIER, DATE_FORMAT(from_unixtime(tl_beachcup_registration.tstamp), '%d.%m.%Y') as DATUM_ANMELDUNG,
                                    ...
                                    p2.surname as NACHNAME_2, p2.name as VORNAME_2, p2.tax_number as STEUER_NR_2, DATE_FORMAT(DATE_ADD(FROM_UNIXTIME(0), INTERVAL p2.birth_date SECOND), '%d.%m.%Y') as GEB_DATUM_2, p2.birth_place as GEB_ORT_2, p2.gender as GESCHLECHT_2, p2.address as ADRESSE_2, p2.zip_code as PLZ_2, p2.city as ORT_2, p2.country as LAND_2, p2.email as EMAIL_2, p2.phone_number as TEL_2, p2.shirt_top as SHIRT_TOP_2, p2.shirt_size as SIZE_2, p2.has_shirt as SHIRT_TOP_ERHALTEN_2, p2.is_fipav as FIPAV_2, p2level.description_de as SPIELER_LEVEL_2, p2.has_medical_certificate as AERZTL_ZEUGNIS_2, p2.is_confirmed as EIGENERKLAERUNG_2,
